#!/usr/bin/env python3
"""
Complete Windows 11 Setup - Handles everything automatically
...
def find_windows11_iso():
    """Find Windows 11 ISO locally"""
    search_paths = [
        Path("isos/windows/windows-11-enterprise-eval.iso"),
        Path("windows-11-enterprise-eval.iso"),
        Path("windows-11.iso"),
        Path.home() / "Downloads" / "windows-11-enterprise-eval.iso",
        Path.home() / "Downloads" / "windows-11.iso",
    ]
    ...
    for root_dir in [Path("."), Path.home() / "Downloads"]:
        if root_dir.exists():
            for iso_file in root_dir.rglob("windows*11*.iso"):
                if iso_file.is_file():
                    return iso_file
...
async def setup_windows11_complete():
    """Complete Windows 11 setup - most stable approach"""
    print("="*70)
    print("  WINDOWS 11 COMPLETE SETUP (Most Stable)")
    print("="*70)
    print()
    ...
    # Step 1: Find Windows 11 ISO locally or on Proxmox
    print("1. Finding Windows 11 ISO...")
    win11_local = find_windows11_iso()
    win11_iso = None
    ...
    # Look for Windows 11 on Proxmox
    ...
    if not win11_iso:
        if win11_local:
            print(f"✅ Found locally: {win11_local.name}")
            
            # Step 2: Upload to Proxmox if not already there
            print("\n2. Uploading to Proxmox...")
            win11_iso_name = win11_local.name
            if win11_iso_name in available_isos:
                print(f"✅ Already on Proxmox: {win11_iso_name}")
                win11_iso = win11_iso_name
            else:
                # Upload it using API or SSH
                print(f"📤 Uploading {win11_iso_name} to Proxmox...")
                win11_iso = await upload_iso_to_proxmox(win11_local, proxmox, settings.proxmox_host, settings.proxmox_password)
                if not win11_iso:
                    ...
                    print(f"   scp {win11_local} root@{settings.proxmox_host}:/var/lib/vz/template/iso/")
                    ...
                    available_isos = [item.get("volid", "").split("/")[-1] for item in storage_content if item.get("content") == "iso"]
                    if win11_iso_name in available_isos:
                        print(f"✅ Found on Proxmox: {win11_iso_name}")
                        win11_iso = win11_iso_name
                    else:
                        print(f"❌ ISO still not found. Please upload manually and re-run.")
                        return False
        else:
            print("❌ Windows 11 ISO not found locally or on Proxmox")
            ...
            print(f"  mv windows-11-enterprise-eval.iso isos/windows/")
            return False
    else:
    ...
    # Disk: 30GB SATA (Windows 11 requires at least 11GB)
    ...
    print(f"✅ Windows 11 ISO: {win11_iso}")
...
if __name__ == "__main__":
    success = asyncio.run(setup_windows11_complete())
