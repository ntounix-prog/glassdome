...
def save_env_file(config):
    """Save configuration to .env file and secrets to secure store"""
    ...
    try:
        ...
        print(f"⚠️  Secrets manager unavailable: {e}")
        print("   Secrets will be stored in .env (less secure)")
    ...
    # Store secrets in secure store
    secrets_stored = []
    if use_secrets_manager:
        if 'PROXMOX_PASSWORD' in existing and existing['PROXMOX_PASSWORD']:
            if secrets.set_secret('proxmox_password', existing['PROXMOX_PASSWORD']):
                secrets_stored.append('proxmox_password')
                # Don't write password to .env
                del existing['PROXMOX_PASSWORD']
        
        if 'PROXMOX_TOKEN_VALUE' in existing and existing['PROXMOX_TOKEN_VALUE']:
            if secrets.set_secret('proxmox_token_value', existing['PROXMOX_TOKEN_VALUE']):
                secrets_stored.append('proxmox_token_value')
                # Don't write token to .env
                del existing['PROXMOX_TOKEN_VALUE']
    
    # Write non-secret config to .env
    with open(env_path, 'w') as f:
        f.write("# Glassdome Configuration\n")
        f.write("# Generated by setup_proxmox.py\n")
        f.write("# Note: Secrets are stored securely, not in this file\n\n")
        ...
        if 'PROXMOX_TOKEN_NAME' in existing:
            ...
            # Token value stored in secrets manager, not in .env
            if not use_secrets_manager:
                f.write(f"PROXMOX_TOKEN_VALUE={existing.get('PROXMOX_TOKEN_VALUE', '')}\n")
        else:
            # Password stored in secrets manager, not in .env
            if not use_secrets_manager and 'PROXMOX_PASSWORD' in existing:
    ...
    if secrets_stored:
        print(f"🔐 Secrets stored securely: {', '.join(secrets_stored)}")
    print(f"\n📝 Non-secret configuration saved to {env_path}")
